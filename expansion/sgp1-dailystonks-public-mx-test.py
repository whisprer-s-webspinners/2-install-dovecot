#!/usr/bin/env python3
"""Send ONE dailystonks.org message from LON1 using its published MX.

Python 3.10+, standard library only. Default: show a preview, no network traffic.
Actual test: python3 sgp1-dailystonks-public-mx-test.py --send

The script queries dailystonks.org MX through Cloudflare's public DNS-over-HTTPS
...
68.183.227.135. The sole recipient is a fresh random address at dailystonks.org.
...
of postmaster@dailystonks.org, and Auto-Submitted: auto-generated. It does not enable
...
RECIPIENT_DOMAIN = "dailystonks.org"
TIMEOUT = 30
...
def published_mx() -> str:
    ...
    req = urllib.request.Request(url, headers={
        "Accept": "application/dns-json", "User-Agent": "sgp1-dailystonks-public-mx-test/" + VERSION,
    }, method="GET")
    try:
    ...
    if records != [(10, MAIL_HOST)]:
        raise TestFailure("Public MX differs from the sole expected repair record; no email sent.")
    return records[0][1]
...
def make_message(test_id: str, now: dt.datetime) -> tuple[str, EmailMessage]:
    ...
    message["From"] = "Mail setup check <postmaster@dailystonks.org>"
    message["To"] = recipient
    message["Date"] = format_datetime(now)
    message["Message-ID"] = f"<sgp1-dailystonks-mx.{test_id}@whispr.dev>"
    message["Subject"] = f"[SGP1 MX test] dailystonks.org - {test_id[:12]}"
    ...
    message.set_content(
        ...
        "this newly generated dailystonks.org address reached your shared inbox.\n\n"
        "LON1 looked up dailystonks.org's public MX, then followed it to SGP1\n"
...
def transmit(recipient: str, message: EmailMessage, mx_host: str) -> int:
    ...
    if (mx_host != MAIL_HOST or not re.fullmatch(r"[a-f0-9]{32}", test_id)
            or not re.fullmatch(r"setup-check-[0-9]{8}-" + test_id + r"@dailystonks\.org", recipient)
...
def main() -> int:
    ...
    parser.add_argument("--send", action="store_true", help="Send exactly one test email to a new dailystonks.org catch-all address.")
    ...
    say("Domain: dailystonks.org; expected MX 10 " + MAIL_HOST + " (" + EXPECTED_IP + ")")
