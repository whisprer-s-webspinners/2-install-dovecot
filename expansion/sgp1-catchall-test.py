#!/usr/bin/env python3
"""Send ONE catch-all delivery test from LON1 directly to SGP1.

Python 3.10+, standard library only. Default: show a preview, no network traffic.
Actual test: python3 sgp1-catchall-test.py --send

The destination is fixed: mail.whispr.dev:25, expected IPv4 68.183.227.135.
The sole recipient is a fresh random address at yt.cafe. This intentionally
tests the prepared receiving route before changing yt.cafe's MX records.
...
def make_message(test_id: str, now: dt.datetime) -> tuple[str, EmailMessage]:
    ...
    message["Message-ID"] = f"<sgp1-catchall.{test_id}@whispr.dev>"
    message["Subject"] = f"[SGP1 test] yt.cafe - {test_id[:12]}"
    ...
    message.set_content(
        ...
        "It did not use or change yt.cafe's public MX records.\n"
...
def transmit(recipient: str, message: EmailMessage) -> int:
...
def main() -> int:
    ...
    say("SGP1 CATCH-ALL DELIVERY TEST " + VERSION)
    ...
    return transmit(recipient, message)
